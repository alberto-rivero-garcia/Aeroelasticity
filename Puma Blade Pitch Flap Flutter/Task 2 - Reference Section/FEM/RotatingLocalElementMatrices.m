function [KK_local,MM_local]=RotatingLocalElementMatrices(xl,xr,Omega,R)
...
waypoints=sort(unique([waypoints_1;waypoints_2])); %coding implementation of the logic checked. 

for i=1:4
    for j=5:7
        KK_local(i,j)=KK_local(i,j)+Omega^2*integral(@(x)dpsidx_cell{i}(x).*m_Puma(x).*ycg_Puma(x).*x.*psi_cell{j}(x),xl,xr,'Waypoints',waypoints);
    end
end
...
for i=5:7
    for j=1:4
        KK_local(i,j)=KK_local(i,j)+Omega^2*integral(@(x)psi_cell{i}(x).*m_Puma(x).*ycg_Puma(x).*x.*dpsidx_cell{j}(x),xl,xr,'Waypoints',waypoints);
    end
end

%Propeller moment: 
[~,waypoints]=Itheta_Puma([xl,xr]);

for i=5:7
    for j=5:7
        KK_local(i,j)=KK_local(i,j)+Omega^2*integral(@(x)psi_cell{i}(x).*Itheta_Puma(x).*psi_cell{j}(x),xl,xr,'Waypoints',waypoints);
    end
end
...
for i=1:4
    for j=5:7
        MM_local(i,j)=MM_local(i,j)+integral(@(x)psi_cell{i}(x).*m_Puma(x).*ycg_Puma(x).*psi_cell{j}(x),xl,xr,'Waypoints',waypoints);
    end
end
...
for i=5:7
    for j=5:7
        MM_local(i,j)=MM_local(i,j)+integral(@(x)psi_cell{i}(x).*Itheta_Puma(x).*psi_cell{j}(x),xl,xr,'Waypoints',waypoints);
    end
end
end
