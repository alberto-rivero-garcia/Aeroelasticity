...
for i=1:B_NKtheta
    ...
    for j=1:(B_NYcg-1) %start from the left
        ...
        if ~Baseline_Grid(right_point,5) %if the point to the right is unsafe
            ...
            eps_waypoints=xco;
            %Compute safety
            [Refined_Grid(B_Npoints+i,5),Refined_Grid(B_Npoints+i,6)]=Task4_wrap_Puma_RefSection(Ne,Nd,Refined_Grid(B_Npoints+i,1),f_eps,eps_waypoints,solparams);
...
for i=2:Nref
    ...
    for j=1:B_NKtheta
        ...
        eps_waypoints=xco;
        %Compute safety
        [Refined_Grid(new_point,5),Refined_Grid(new_point,6)]=Task4_wrap_Puma_RefSection(Ne,Nd,Refined_Grid(new_point,1),f_eps,eps_waypoints,solparams);
    end
end
