...
%v5 relies on an updated wrapper that is better suited to handle modes with
%zero frequencies. 
...
Nd=6;
solparams.Nomega=10;
solparams.maxiter=100; %ample to allow for complete convergence everywhere. 
solparams.tol=1e-6;
...
for i=1:(NKtheta*Ncg)
    ...
    eps_waypoints=xco;
    %Compute safety
    [Baseline_Grid(i,5),Baseline_Grid(i,6)]=Task4_wrap_Puma_RefSection(Ne,Nd,Baseline_Grid(i,1),f_eps,eps_waypoints,solparams);
end
