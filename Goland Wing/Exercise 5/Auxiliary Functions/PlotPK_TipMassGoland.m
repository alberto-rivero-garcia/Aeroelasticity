function PlotPK_TipMassGoland(Ne,Nd,Umax,Nu,xmhat)
%This function wrapper provides the frequency and damping versus airspeed
%plots for a Goland wing with a tip mass. It uses Ne elements, Nd modes and
%plots a velocity range from 0 to Umax with Nu uniformly distributed
%points. The chordiwse position of the mass' CoG, relative to the chord, is
%specified as xmhat. 
...
[fr,qr,FEMdata]=FEM_TipMassCoupledBendingTorsion(Ne,Nd,xmhat,0);
...
for i=1:Nd
    curvelabel='Mode' + string(i);
    plot(UU,g(i,:),'DisplayName',curvelabel)
    hold on
end
plot(UU,zeros(size(UU)),'r--')
grid minor
legend
xlabel('$U \; [m/s]$','Interpreter','latex')
