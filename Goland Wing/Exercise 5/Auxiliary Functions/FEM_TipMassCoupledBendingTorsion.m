function [fr,qr,FEMdata]=FEM_TipMassCoupledBendingTorsion(Ne,Nd,xmhat,Display)
%This function wrapper uses the procedure defined in CoupledBendingTorsion.m 
%to compute coupled bending-torsion natural frequencies and modes, with the
% addition of a tip mass whose CoG occupies the chordwise (non-dimensional)
% position given by xmhat. It receives a prescribed number of elements (which
% are taken as joint bending-torsion elements) and the number of modes of 
% interest, as well as xmhat. Display is a logical value that can be used if 
% frequencies and modes are to be displayed.  
...
s=shat*c;
Itheta=Io+m*s^2;
xe=0.33*c;      %[m] chordwise position of elastic axis

%Tip mass: 
mtip=80;    %[kg]
Itip=15;    %[kg m^2]
xm=xmhat*c; %[m] chordwise position of tip mass' center of gravity. 
...
%Introduce terms due to tip mass: 
MM(NdofsB-1,NdofsB-1)=MM(NdofsB-1,NdofsB-1)+mtip; % at the bending displacement of the tip dof.
MM(end,end)=MM(end,end)+Itip+mtip*(xe-xm)^2;      % at the torsion angle of the tip dof. 
MM(NdofsB-1,end)=MM(NdofsB-1,end)+(xe-xm)*mtip;   % effect on the bending displacement of the tip caused by the torsion of the tip.
MM(end,NdofsB-1)=MM(end,NdofsB-1)+mtip*(xe-xm);   % effect on the tip torsion caused by tip bending displacement. 
...
if Display
    display(fr)
    figure(1)
    yplot=linspace(0,L,1000);
    for i=1:Nd
        curvelabel='Mode'+string(i);
        plot(yplot,EvaluateCoupledBendingDisplacement(yplot,qr(:,i),deltay,Connect),'DisplayName',curvelabel)
        hold on
    end
    title('Bending Displacement for Coupled Modes')
    legend
    ...
    for i=1:Nd
        curvelabel='Mode'+string(i);
        plot(yplot,EvaluateCoupledTorsionDisplacement(yplot,qr(:,i),deltay,Connect),'DisplayName',curvelabel)
        hold on
    end
    title('Torsion Displacement for Coupled Modes')
    legend
end
