function [fr,qr,FEMdata]=SensitivityFEM_CoupledBendingTorsion(Ne,Nd,params)
...
%elements) and the number of modes of interest. Modified to ease
%sensitivity analyses by receiving a parameter structure
L=6.096;        %[m]
c=1.829;        %[m]
ghat=params.ghat; %[-] Chordwise Position of the center of gravity (non-dimensional) 
EI=params.EI;    %[N*m2]
m=35.72;        %[kg/m]
GJ=params.GJ;      %[N m^2]
Io=7.452;       %[kg m] 
s=(ghat-0.33)*c; %some hardcoded elastic axis position, it will suffice for now.
Itheta=Io+m*s^2;
